...
export function AgentItem({ agent, isActive }: AgentItemProps) {
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2 hover:bg-white/5 transition-colors">
      <div className="relative flex h-8 w-8 items-center justify-center bg-amber-500 text-xs font-bold text-white">
        {agent.name[0]}
        {isActive && (
          <span className="absolute -right-0.5 -top-0.5 h-2.5 w-2.5 border-2 border-[#0a0a0a] bg-emerald-500" />
        )}
      </div>
      <div className="min-w-0">
